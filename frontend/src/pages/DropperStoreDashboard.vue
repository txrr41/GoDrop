    ...
    <div v-show="tab === 'products'" class="tab-panel">
      ...
      <div v-else>
        ...
        <DropperProductSelector
            :initial-product-ids="store.products?.map(p => p.id) ?? []"
...
const managingProducts = ref(false)   // ← controla se mostra seletor ou grade
...
// Chamado pelo DropperProductSelector ao salvar
async function onProductsUpdated(newIds) {
  // Recarrega a loja para refletir os novos produtos no banner/stat
...
.store-status-row { display: flex; align-items: center; gap: 8px; }
...
.store-slogan { font-size: 14px; color: rgba(255,255,255,0.75); }
...
.banner-btn.primary { background: white; color: #0F172A; }
.banner-btn.primary:hover { background: #F1F5F9; }
.banner-btn.secondary {
  background: rgba(255,255,255,0.15);
  color: white;
  border: 1px solid rgba(255,255,255,0.25);
}
.banner-btn.secondary:hover { background: rgba(255,255,255,0.25); }
...
.slug-row strong { color: white; }

.toggle-wrap { display: flex; align-items: center; gap: 10px; cursor: pointer; }
.toggle-label { font-size: 13px; font-weight: 500; color: rgba(255,255,255,0.8); }
...
.sc-num   { font-size: 20px; font-weight: 700; color: #0F172A; }
...
.panel-sub   { font-size: 13px; color: #64748B; margin-top: 3px; }
...
.manage-btn:hover { border-color: #94A3B8; color: #0F172A; }

.empty-state { text-align: center; padding: 56px 0; }
.es-icon  { font-size: 48px; margin-bottom: 12px; }
.es-title { font-size: 16px; font-weight: 600; color: #0F172A; margin-bottom: 6px; }
.es-sub   { font-size: 14px; color: #64748B; }
...
.pc-img-wrap { position: relative; height: 120px; overflow: hidden; }
.pc-img {
...
.pc-price { font-size: 13px; font-weight: 700; }
...
.appearance-form { display: flex; flex-direction: column; gap: 24px; }
...
.colors-row { display: flex; gap: 20px; flex-wrap: wrap; }
...
.cf-preview.bordered { border: 1.5px solid #E2E8F0; }
.cf-input {
  position: absolute; inset: 0;
  width: 100%; height: 100%;
...
.cf-hex   { font-size: 11px; color: #94A3B8; font-family: monospace; }
...
.save-btn:hover { background: #1E293B; box-shadow: 0 4px 12px rgba(15,23,42,0.25); }
.save-btn.loading { opacity: 0.7; cursor: wait; }
...
/* ── LIVE PREVIEW ── */
.appearance-preview { position: sticky; top: 24px; }
.ap-label {
  font-size: 11px; font-weight: 700;
  letter-spacing: 0.08em; text-transform: uppercase; color: #94A3B8;
  margin-bottom: 12px;
...
.apb-dots { display: flex; gap: 5px; }
.apb-dots span { width: 9px; height: 9px; border-radius: 50%; }
...
.apb-hero { padding: 20px 18px; transition: background 0.4s; }
.apb-name {
  display: block;
  font-family: 'Instrument Serif', serif;
  font-size: 18px;
...
.apbp-img { height: 64px; transition: background 0.4s; }
.apbp-info { padding: 8px; display: flex; flex-direction: column; gap: 4px; }
