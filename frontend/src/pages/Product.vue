  ...
  <div class="pm-root">
    ...
    <section class="pm-stats">
      <div class="stat-card">
        ...
        <div class="stat-body"><span class="stat-value">{{ products.length }}</span><span class="stat-label">Total de Produtos</span>
        </div>
      ...
      <div class="stat-card">
        ...
        <div class="stat-body"><span class="stat-value">{{ totalStock.toLocaleString('pt-BR') }}</span><span
            class="stat-label">Estoque Total</span></div>
      ...
      <div class="stat-card">
        ...
        <div class="stat-body"><span class="stat-value">{{ formatCurrency(totalValue) }}</span><span class="stat-label">Valor em Estoque</span>
        </div>
      ...
      <div class="stat-card">
        ...
        <div class="stat-body"><span class="stat-value">{{ lowStockCount }}</span><span class="stat-label">Estoque Crítico</span>
        </div>
    ...
    <section class="pm-toolbar">
      <div class="pt-filters">
        <button v-for="cat in categoryTabs" :key="cat" class="pt-tab"
                :class="{ 'pt-tab--active': selectedCategory === cat }" @click="selectedCategory = cat">
          {{ cat === 'all' ? 'Todos' : cat }}
        </button>
      </div>
    ...
    <section v-if="viewMode === 'grid'" class="pm-grid">
      <div v-for="product in filteredProducts" :key="product.id" class="pg-card"
           :class="{ 'pg-card--featured': product.destaque }">
        <div class="pg-image-wrap">
          <img :src="product.imagem || 'https://via.placeholder.com/400x300/f1f5f9/94a3b8?text=Sem+Foto'"
               :alt="product.nome" class="pg-image"/>
          <div class="pg-badge-wrap">
            <span v-if="product.destaque" class="pg-badge pg-badge--star">★ Destaque</span>
            <span class="pg-badge" :class="getStockBadgeClass(product.estoque)">{{
    ...
    <section v-else class="pm-list">
      <table class="pl-table">
        ...
        <tr v-for="product in filteredProducts" :key="product.id" class="pl-row">
          <td>
            <div class="pl-product">
              ...
              <div>
                <div class="pl-name">{{ product.nome }}</div>
                <div class="pl-desc-sm">{{ product.descricao }}</div>
              </div>
            </div>
          </td>
          ...
          <td><span class="pl-status" :class="getStockBadgeClass(product.estoque)">{{
              getStockLabel(product.estoque)
            }}</span></td>
          <td>
        ...
        <tr v-if="filteredProducts.length === 0">
          <td colspan="6" class="pl-empty">Nenhum produto encontrado</td>
        </tr>
        </tbody>
      </table>
    </section>
      ...
      <div class="modal-panel">
        <div class="modal-head">
          <div>
            ...
            <p class="modal-sub">
              {{ editingProduct ? 'Atualize as informações do produto' : 'Preencha os dados para cadastrar' }}</p>
          </div>
        ...
        <div class="modal-body">
          <div class="modal-col-img">
            ...
            <div class="img-drop" :class="{ 'img-drop--has': formData.image }" @click="triggerFileInput"
            ...
            <div class="destaque-toggle" @click="formData.destaque = !formData.destaque">
              <div class="dt-info">
                ...
                <div><p class="dt-title">Produto em Destaque</p>
                  <p class="dt-sub">Aparece na página inicial</p></div>
              </div>
              <div class="dt-switch" :class="{ 'dt-switch--on': formData.destaque }">
          ...
          <div class="modal-col-form">
            <div class="mf-group">
              ...
              <input v-model="formData.name" class="mf-input" :class="{ 'mf-input--error': errors.name }"
                     placeholder="Ex: Notebook Pro 15"/>
              <span v-if="errors.name" class="mf-error">{{ errors.name }}</span>
            </div>
            <div class="mf-group">
              <label class="mf-label">Descrição</label>
              <textarea v-model="formData.description" class="mf-input mf-textarea" placeholder="Descreva o produto..."
                        rows="4"></textarea>
            </div>
            <div class="mf-row">
              <div class="mf-group">
                ...
                <div class="mf-input-wrap">
                  ...
                  <input v-model.number="formData.price" type="number" step="0.01" min="0"
                         class="mf-input mf-input--prefixed" :class="{ 'mf-input--error': errors.price }"
                         placeholder="0,00"/>
                </div>
              ...
              <div class="mf-group">
                ...
                <input v-model.number="formData.stock" type="number" min="0" class="mf-input"
                       :class="{ 'mf-input--error': errors.stock }" placeholder="0"/>
            ...
            <div v-if="formData.price > 0" class="mf-preview">
              <div class="mfp-row"><span>Preço de venda</span><strong>{{ formatCurrency(formData.price) }}</strong>
              </div>
              <div v-if="formData.stock > 0" class="mfp-row"><span>Valor total em estoque</span><strong>{{
    ...
    <div v-if="showCatModal" class="modal-backdrop" @click.self="showCatModal = false">
      <div class="modal-panel modal-panel--sm">
        <div class="modal-head">
          <div><h2 class="modal-title">Nova Categoria</h2>
            <p class="modal-sub">Organize seus produtos por categoria</p></div>
    ...
    <div v-if="showDeleteModal" class="modal-backdrop" @click.self="showDeleteModal = false">
      <div class="modal-panel modal-panel--sm">
        <div class="modal-del-body">
          ...
          <p class="modal-sub" style="margin-top:8px">Tem certeza que deseja excluir <strong>{{
              deletingProduct?.nome
            }}</strong>? Esta ação não pode ser desfeita.</p>
        </div>
        <div class="modal-foot">
...
const saveProduct = async () => {
  ...
  const payload = {
    ...
    categoria: formData.category || 'Sem categoria',
...
const handleFileChange = e => {
  ...
  r.onload = ev => {
    formData.image = ev.target.result
  }
